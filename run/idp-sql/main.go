...
// Sample idp-sql demonstrates handling end-user authentication and SQL
// database integration from Cloud Run.
package main
...
func newApp() *app {
	...
	// TODO: Create $table instead of votes.
	// Create the table if it does not already exist.
	if _, err = app.db.Exec(`CREATE TABLE IF NOT EXISTS votes
	( vote_id SERIAL NOT NULL, time_cast timestamp NOT NULL,
	candidate VARCHAR(6) NOT NULL, uid VARCHAR(128) NOT NULL, PRIMARY KEY (vote_id) );`); err != nil {
...
func recentVotes(app *app) ([]vote, error) {
	var votes []vote
	rows, err := app.db.Query(`SELECT candidate, time_cast, uid FROM votes ORDER BY created_at DESC LIMIT 5`)
	if err != nil {
...
func currentTotals(app *app) (*templateData, error) {
	...
	var catVotes, dogVotes uint
	err := app.db.QueryRow(`SELECT count(vote_id) FROM votes WHERE candidate='CATS'`).Scan(&catVotes)
	if err != nil {
		return nil, fmt.Errorf("DB.QueryRow: %v", err)
	}
	err = app.db.QueryRow(`SELECT count(id) FROM votes WHERE candidate='DOGS'`).Scan(&dogVotes)
	if err != nil {
		return nil, fmt.Errorf("DB.QueryRow: %v", err)
	}

	var voteDiffStr string = voteDiff(int(math.Abs(float64(catVotes) - float64(dogVotes)))).String()
...
func saveVote(w http.ResponseWriter, r *http.Request, app *app) error {
	...
	// TODO Extract & validate uid
	uid := 42

	// [START cloud_sql_postgres_databasesql_connection]
	sqlInsert := "INSERT INTO votes(candidate, time_cast, uid) VALUES($1, NOW(), $2)"
	if team == "cat" || team == "dog" {
		if _, err := app.db.Exec(sqlInsert, team, uid); err != nil {
