...
func createCluster(w io.Writer, projectID, region, clusterName string) error {
	...
	clusterClient, err := dataproc.NewClusterControllerClient(ctx,
		option.WithEndpoint(region+"-dataproc.googleapis.com:443"),
	)
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("Error submitting the cluster creation request: %s", err)
	}

	resp, err := op.Wait(ctx)
	if err != nil {
		return fmt.Errorf("Error creating the cluster: %s", err)
	}
