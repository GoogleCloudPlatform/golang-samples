...
func setClusterName(tc testutil.Context) {
	clusterName = tc.ProjectID + "-test-dataproc-create-cluster"
}

func teardown(t *testing.T, tc testutil.Context) {
	ctx := context.Background()

	a := fmt.Sprintf("%s-dataproc.googleapis.com:443", region)
	clusterClient, err := dataproc.NewClusterControllerClient(ctx, option.WithEndpoint(a))
