...
type backupScheduleSampleFunc func(ctx context.Context, w io.Writer, dbName string, scheduleId string) error
...
func TestBackupScheduleSample(t *testing.T) {
	...
	var out string
	out = runBackupScheduleSample(ctx, t, createFullBackupSchedule, dbName, "full-backup-schedule", "failed to create full backup schedule")
	...
	out = runBackupScheduleSample(ctx, t, createIncrementalBackupSchedule, dbName, "incremental-backup-schedule", "failed to create incremental backup schedule")
	...
	out = runSampleWithContext(ctx, t, listBackupSchedules, dbName, "failed to list backup schedule")
	assertContains(t, out, fmt.Sprintf("%s/backupSchedules/%s", dbName, "full-backup-schedule"))
	assertContains(t, out, fmt.Sprintf("%s/backupSchedules/%s", dbName, "incremental-backup-schedule"))

	out = runBackupScheduleSample(ctx, t, getBackupSchedule, dbName, "full-backup-schedule", "failed to get backup schedule")
	assertContains(t, out, fmt.Sprintf("%s/backupSchedules/%s", dbName, "full-backup-schedule"))

	out = runBackupScheduleSample(ctx, t, updateBackupSchedule, dbName, "full-backup-schedule", "failed to update backup schedule")
	assertContains(t, out, "Updated backup schedule")
	assertContains(t, out, fmt.Sprintf("%s/backupSchedules/%s", dbName, "full-backup-schedule"))

	out = runBackupScheduleSample(ctx, t, deleteBackupSchedule, dbName, "full-backup-schedule", "failed to delete backup schedule")
...
func runBackupScheduleSample(ctx context.Context, t *testing.T, f backupScheduleSampleFunc, dbName string, scheduleId string, errMsg string) string {
	var b bytes.Buffer
	if err := f(ctx, &b, dbName, scheduleId); err != nil {
		t.Errorf("%s: %v", errMsg, err)
	}
	return b.String()
}
