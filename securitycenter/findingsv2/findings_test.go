...
func TestUpdateFindingSourceProperties(t *testing.T) {
    ...
    testutil.Retry(t, 5, 5*time.Second, func(r *testutil.R) {
        ...
        finding, err := createTestFinding(ctx, client, "update-finding", "MEDIUM_RISK_ONE")
        if err != nil {
...
func TestSetFindingState(t *testing.T) {
    ...
    testutil.Retry(t, 5, 5*time.Second, func(r *testutil.R) {
        ...
        finding, err := createTestFinding(ctx, client, "set-finding-state", "LOW_RISK")
        if err != nil {
            r.Errorf("Failed to create finding: %v", err)
        }
