...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Filter is not a valid regexp: %w", err)
		os.Exit(2)
	}
	_ = filterRE
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Could not create DefaultClient: %w", err)
		os.Exit(1)
	}
	gae, err = appengine.New(hc)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Could not create App Engine service: %w", err)
		os.Exit(1)
	}
	...
	if *service != "" {
		...
		if err := gae.Apps.Services.List(*proj).Pages(ctx, func(lsr *appengine.ListServicesResponse) error {
			...
			fmt.Fprintf(os.Stderr, "Could not list App Engine services: %w", err)
			os.Exit(1)
		}
	}
