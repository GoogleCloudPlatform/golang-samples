...
// resumeAnywhereCache resumes a paused Anywhere Cache.
func resumeAnywhereCache(w io.Writer, bucket, zone string) error {
	...
	resp, err := client.ResumeAnywhereCache(ctx, req)
	if err != nil {
		return fmt.Errorf("ResumeAnywhereCache: %w", err)
	}

	fmt.Fprintf(w, "resumed anywhere cache with path %q", resp.Name)
	return nil
}
