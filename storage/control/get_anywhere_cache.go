...
// getAnywhereCache gets an Anywhere Cache for the bucket in the specified zone.
func getAnywhereCache(w io.Writer, bucket, zone string) error {
	// bucket := "bucket-name"
	// zone := "us-central1-f"
	...
	resp, err := client.GetAnywhereCache(ctx, req)
	if err != nil {
		return fmt.Errorf("GetAnywhereCache: %w", err)
	}

	fmt.Fprintf(w, "got anywhere cache with path %q", resp.Name)
	return nil
}
