...
var collaborators = [2]collaborator{
	...
	"<RESULTS_BUCKET_NAME>",         // The name of Alex's results bucket
	"<RESULTS_OUTPUT_FILE>",         // The name of Alex's output file that contains the results
	},
	{
	...
	"<RESULTS_BUCKET_NAME>",             // The name of Bola's results bucket
	"<RESULTS_OUTPUT_FILE>",             // The name of Bola's output file that contains the results
	},
}
...
func main() {
	...
	storageClient, err := storage.NewClient(ctx) // using the default credential on the gce VM
	if err != nil {
	panic(err)
	}
