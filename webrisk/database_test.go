	...
	vectors := []struct {
		...
		newDB: &database{
			...
			tfl: threatsForLookup{
				ThreatType_Unspecified: newHashSet([]hashPrefix{"aaaa", "bbbb"}),
				ThreatType_Malware: newHashSet([]hashPrefix{"bbbb", "cccc"}),
			},
		},
	}, {
		...
		config: &Config{
			ThreatLists: []ThreatType{ThreatType_Unspecified},
		},
		oldDB: &database{
			last: now,
			tfu: threatsForUpdate{
				...
				ThreatType_UnwantedSoftware: partialHashes{
...
func TestDatabaseUpdate(t *testing.T) {
	var (
		td013 = ThreatType(ThreatType_Unspecified)
		td114 = ThreatType(ThreatType_Malware)
	...
	resp = newResp(td114, partial, []int32{0, 1, 2, 3}, nil,
	...
	now = now.Add(time.Hour)
	resp = newResp(td114, full, nil, []string{"aaaa", "0421e", "666666", "7777777", "88888888"},
	...
	now = now.Add(time.Hour)
	resp = newResp(td114, partial, nil, nil,
				"d1", "a3b93fac424834c2447e2dbe5db3ec8553519777523907ea310e207f556a7637")
	delay, updated = db.Update(context.Background(), mockAPI)
	if db.err != nil || !updated {
	...
	now = now.Add(time.Hour)
	resp = newResp(td114, full, nil, []string{"AAAA", "0421E"},
	...
	now = now.Add(time.Hour)
	resp = newResp(td013, partial, []int32{0, 1}, []string{"fizz", "buzz"},
	...
	now = now.Add(time.Hour)
	resp = newResp(td013, partial, []int32{9000}, []string{"fizz", "buzz"},
...
func TestDatabaseLookup(t *testing.T) {
	var (
		td0 = ThreatType(ThreatType_Unspecified)
		td1 = ThreatType(ThreatType_Malware)
		td2 = ThreatType(2)
		td3 = ThreatType(3)
	)
	...
	vectors := []struct {
		...
		output:  "59b8",
		threats: []ThreatType{td3},
	}, {
		input:   "524dfa307ba397754a35dcce0ee5f54a",
		output:  "524d",
		threats: []ThreatType{td0, td3},
	}, {
