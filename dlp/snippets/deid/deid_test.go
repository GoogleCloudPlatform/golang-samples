...
func createDeidentifiedTemplate(projectID, deidentifyTemplateID string) error {
	...
	log.Println("[info] [createDeidentifiedTemplate] " + "template " + resp.Name + "is created")
	return nil
}
...
func createStructuredDeidentifiedTemplate(projectID, deidentifyStructuredTemplateID string) error {
	...
	log.Println("[info] [createStructuredDeidentifiedTemplate] " + "template " + resp.Name + "is created")
	return nil
}
...
func createRedactImageTemplate(projectID, redactImageTemplate string) error {
	...
	log.Println("[info] [createRedactImageTemplate] " + "template " + resp.Name + "is created")
	return nil
}
...
func deleteTemplate(projectID string) error {
	...
	for _, v := range abc {
		...
		log.Printf("[info] [deleteTemplate] deleted a template : %s", v)
	}
	return nil
}
...
func createBucket(projectID, bucketName string) error {
	...
	if !bucketExists {
		...
		log.Printf("[info] [createBucket] ---Bucket '%s' created successfully.\n", bucketName)
	} else {
		log.Printf("[info] [createBucket] ---Bucket '%s' already exists.\n", bucketName)
	}
	log.Println("[info] [createBucket] createbucket function is executed-------")
	return nil
}
...
func deleteBucket(projectID, bucketName string) error {
	...
	for {
		...
		if err != nil {
			log.Fatalf("[info] [deleteBucket] Failed to list objects in bucket: %v", err)
		}

		// Delete each object in the bucket.
		if err := bucket.Object(objAttrs.Name).Delete(ctx); err != nil {
			log.Fatalf("[info] [deleteBucket] Failed to delete object %s: %v", objAttrs.Name, err)
		}
		log.Printf("[info] [deleteBucket] Deleted object: %s\n", objAttrs.Name)
	}
	if err := bucket.Delete(ctx); err != nil {
		log.Fatalf("[info] [deleteBucket] Failed to delete bucket: %v", err)
	}
