...
func TestDeleteRegionalSecretAnnotation(t *testing.T) {
	...
	var b bytes.Buffer
	if err := deleteRegionalSecretAnnotation(&b, secret.Name, locationID, annotationKey); err != nil {
		t.Fatal(err)
	}
...
func TestBindTagToRegionalSecret(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Failed to create tag bindings client for verification: %v", err)
	}
...
func TestDeleteRegionalSecretVersionDestroyTTL(t *testing.T) {
	...
	b.Reset()
	if err := deleteRegionalSecretVersionDestroyTTL(&b, tc.ProjectID, secretID, locationID); err != nil {
		t.Fatal(err)
	}
