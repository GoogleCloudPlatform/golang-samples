...
func bindTagsToSecret(w io.Writer, projectId, secretId, tagValue string) error {
	...
	newSecret, err := smClient.CreateSecret(ctx, createReq)
	if err != nil {
	...
	bindingReq := &resourcemanagerpb.CreateTagBindingRequest{
		TagBinding: &resourcemanagerpb.TagBinding{
			Parent:   fmt.Sprintf("//secretmanager.googleapis.com/%s", newSecret.Name),
			TagValue: tagValue,
		},
	}

	op, err := tagBindingsClient.CreateTagBinding(ctx, bindingReq)
	if err != nil {
		return fmt.Errorf("failed to start create tag binding operation: %w", err)
	}

	if _, err = op.Wait(ctx); err != nil {
		return fmt.Errorf("failed to wait for tag binding operation: %w", err)
	}

	fmt.Fprintf(w, "Tag binding created for secret %s with tag value %s\n", newSecret.Name, tagValue)
	return nil
}
