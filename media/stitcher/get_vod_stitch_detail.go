...
// getVodStitchDetail gets the specified stitch detail for a VOD session.
func getVodStitchDetail(w io.Writer, projectID, location, sessionID, stitchDetailID string) error {
	...
	if err != nil {
		return fmt.Errorf("GetVodStitchDetail: %v", err)
	}

	fmt.Fprintf(w, "VOD stitch detail: %v", response.Name)
	return nil
}
