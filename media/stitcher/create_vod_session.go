...
// createVodSession creates a VOD session. VOD sessions are ephemeral resources that expire after a few hours.
func createVodSession(w io.Writer, projectID, location, sourceURI, adTagURI string) error {
	...
	if err != nil {
		return fmt.Errorf("CreateVodSession: %v", err)
	}
