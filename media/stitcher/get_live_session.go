...
// getLiveSession gets a previously-created live session.
func getLiveSession(w io.Writer, projectID, location, sessionID string) error {
	...
	req := &stitcherstreampb.GetLiveSessionRequest{
	...
	if err != nil {
		return fmt.Errorf("GetLiveSession: %v", err)
	}
