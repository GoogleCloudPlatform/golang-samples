...
func TestStitcher(t *testing.T) {
	...
	testCdnKeys(t, projectNumber)
	t.Logf("\ntestCdnKeys() completed\n")

	testVodSessions(t, projectNumber, vodURI, vodAdTagURI)
	t.Logf("\ntestVodSessions() completed\n")

	testLiveSessions(t, projectNumber, liveURI, liveAdTagURI, slateURI)
	t.Logf("\ntestLiveSessions() completed\n")
}
...
// delete operation checks for a hard-coded string response.
func testCdnKeys(t *testing.T, projectNumber string) {
	tc := testutil.SystemTest(t)
	buf := &bytes.Buffer{}
	...
	if err := getCdnKey(buf, tc.ProjectID, location, gcdnCdnKeyID); err == nil {
		testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
			if err := deleteCdnKey(buf, tc.ProjectID, location, gcdnCdnKeyID); err != nil {
	...
	if err := getCdnKey(buf, tc.ProjectID, location, akamaiCdnKeyID); err == nil {
		testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
			if err := deleteCdnKey(buf, tc.ProjectID, location, akamaiCdnKeyID); err != nil {
				r.Errorf("deleteCdnKey got err: %v", err)
			}
		})
	}
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		...
		if err := createCdnKey(buf, tc.ProjectID, location, gcdnCdnKeyID, hostname, gcdnKeyname, gcdnPrivateKey, ""); err != nil {
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		...
		if err := listCdnKeys(buf, tc.ProjectID, location); err != nil {
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		cdnKeyName := fmt.Sprintf("projects/%s/locations/%s/cdnKeys/%s", tc.ProjectID, location, gcdnCdnKeyID)
		if err := updateCdnKey(buf, tc.ProjectID, location, gcdnCdnKeyID, updatedHostname, gcdnKeyname, updatedGcdnPrivateKey, ""); err != nil {
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		...
		if err := createCdnKey(buf, tc.ProjectID, location, akamaiCdnKeyID, hostname, "", "", akamaiTokenKey); err != nil {
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		cdnKeyName := fmt.Sprintf("projects/%s/locations/%s/cdnKeys/%s", tc.ProjectID, location, akamaiCdnKeyID)
		if err := updateCdnKey(buf, tc.ProjectID, location, akamaiCdnKeyID, updatedHostname, "", "", updatedAkamaiTokenKey); err != nil {
			r.Errorf("updateCdnKey got err: %v", err)
		}
		if got := buf.String(); !strings.Contains(got, cdnKeyName) {
			r.Errorf("updateCdnKey got\n----\n%v\n----\nWant to contain:\n----\n%v\n----\n", got, cdnKeyName)
...
func testVodSessions(t *testing.T, projectNumber, vodURI, vodAdTagURI string) {
	tc := testutil.SystemTest(t)
	buf := &bytes.Buffer{}
	sessionID := ""
	...
	if err := createVodSession(buf, tc.ProjectID, location, vodURI, vodAdTagURI); err != nil {
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		...
		if err := getVodSession(buf, tc.ProjectID, location, sessionID); err != nil {
	...
	if err := listVodAdTagDetails(buf, tc.ProjectID, location, sessionID); err != nil {
	...
	testutil.Retry(t, 1, 2*time.Second, func(r *testutil.R) {
		if err := getVodAdTagDetail(buf, tc.ProjectID, location, sessionID, adTagDetailsID); err != nil {
	...
	if err := listVodStitchDetails(buf, tc.ProjectID, location, sessionID); err != nil {
	...
	testutil.Retry(t, 1, 2*time.Second, func(r *testutil.R) {
		if err := getVodStitchDetail(buf, tc.ProjectID, location, sessionID, stitchDetailsID); err != nil {
...
func testLiveSessions(t *testing.T, projectNumber, liveURI, liveAdTagURI, slateURI string) {
	tc := testutil.SystemTest(t)
	buf := &bytes.Buffer{}
	sessionID := ""
	...
	playURI, err := createLiveSession(buf, tc.ProjectID, location, liveURI, liveAdTagURI, slateID)
	if err != nil {
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		...
		if err := getLiveSession(buf, tc.ProjectID, location, sessionID); err != nil {
	...
	if err := listLiveAdTagDetails(buf, tc.ProjectID, location, sessionID); err != nil {
	...
	// TODO: b/231626944 for projectNumber below
	testutil.Retry(t, 1, 2*time.Second, func(r *testutil.R) {
		if err := getLiveAdTagDetail(buf, projectNumber, location, sessionID, adTagDetailsID); err != nil {
