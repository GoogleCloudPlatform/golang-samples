...
func TestListVodAdTagDetails(t *testing.T) {
	...
	adTagDetailsNamePrefix := fmt.Sprintf("/locations/%s/vodSessions/%s/vodAdTagDetails/", location, sessionID)

	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		if err := listVodAdTagDetails(&buf, tc.ProjectID, sessionID); err != nil {
			r.Errorf("listVodAdTagDetails got err: %v", err)
		}
		if got := buf.String(); !strings.Contains(got, adTagDetailsNamePrefix) {
			r.Errorf("listVodAdTagDetails got: %v Want to contain: %v", got, adTagDetailsNamePrefix)
		}
	})
}
