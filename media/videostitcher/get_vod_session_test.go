...
func TestGetVodSession(t *testing.T) {
	...
	vodSession := fmt.Sprintf("/locations/%s/vodSessions/%s", location, sessionID)

	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		if err := getVodSession(&buf, tc.ProjectID, sessionID); err != nil {
			r.Errorf("getVodSession got err: %v", err)
		}
		if got := buf.String(); !strings.Contains(got, vodSession) {
			r.Errorf("getVodSession got: %v Want to contain: %v", got, vodSession)
		}
	})
}
