...
func setupTestCreateLiveConfig(slateID string, t *testing.T) {
	t.Helper()
...
func TestCreateLiveConfig(t *testing.T) {
	tc := testutil.SystemTest(t)
	...
	setupTestCreateLiveConfig(slateID, t)
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		if err := createLiveConfig(&buf, tc.ProjectID, liveConfigID, liveURI, slateID); err != nil {
			r.Errorf("createLiveConfig got err: %v", err)
		}
		if got := buf.String(); !strings.Contains(got, liveConfigName) {
			r.Errorf("createLiveConfig got: %v Want to contain: %v", got, liveConfigName)
		}
	})

	t.Cleanup(func() {
		teardownTestCreateLiveConfig(liveConfigName, slateName, t)
	})
}

func teardownTestCreateLiveConfig(liveConfigName, slateName string, t *testing.T) {
	t.Helper()
	ctx := context.Background()
	client, err := stitcher.NewVideoStitcherClient(ctx)
	if err != nil {
	...
	op2, err2 := client.DeleteSlate(ctx, req2)
	if err2 != nil {
		t.Errorf("client.DeleteSlate: %v", err2)
	}
	err2 = op2.Wait(ctx)
	if err2 != nil {
		t.Error(err2)
