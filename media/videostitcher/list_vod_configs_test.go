...
func TestListVodConfigs(t *testing.T) {
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		if err := listVodConfigs(&buf, tc.ProjectID); err != nil {
			r.Errorf("listVodConfigs got err: %v", err)
		}
		if got := buf.String(); !strings.Contains(got, vodConfigName) {
			r.Errorf("listVodConfigs got: %v Want to contain: %v", got, vodConfigName)
		}
	})
