...
func setupTestListLiveConfigs(slateID, liveConfigID string, t *testing.T) func() {
	t.Helper()
...
func TestListLiveConfigs(t *testing.T) {
	tc := testutil.SystemTest(t)
	...
	teardown := setupTestListLiveConfigs(slateID, liveConfigID, t)
	t.Cleanup(teardown)
	...
	testutil.Retry(t, 3, 2*time.Second, func(r *testutil.R) {
		...
		if got := buf.String(); !strings.Contains(got, liveConfigName) {
			r.Errorf("listLiveConfigs got: %v Want to contain: %v", got, liveConfigName)
		}
	})
}
